...
public:
    ...
    SceneData requireSceneData(VulkanDescriptorPool& descriptorPool, uint32_t threadCount, 
    ...
    VulkanDescriptorSet& requireDescriptorSet(VulkanDescriptorPool& descriptorPool, const BindingMap<VkDescriptorBufferInfo>& bufferInfos, const BindingMap<VkDescriptorImageInfo>& imageInfos);
    VulkanDescriptorPool& requireDescriptorPool(const VulkanDescriptorSetLayout& layout, uint32_t poolSize);
...
private:
    ...
    std::unordered_set<std::unique_ptr<VulkanDescriptorPool>> descriptorPoolSet;
